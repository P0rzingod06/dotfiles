...
where 1=1 
--and program_id != 97
--and last_update_Date > sysdate - 33.87
--and last_update_date < sysdate - 33.83
--and creation_Date < '10/07/2014 3:00:00 PM'
order by creation_date desc

select * from wwt_orig_order_headers
where 1=1
--and header_id IN
--and ship_from_org_id = 582
--and creation_date > sysdate - 34
--and salesrep_id = '-3'
...
and      object_name like '%ASN%OUTBOUND%'

select * from WWT_ASN_OUTBOUND_shipments
where 1=1
--and salesrep_id = -3
--and sales_order_num = 84083
order by creation_date desc
...
and lookup_id = 369168

update oe_order_headers_all
set ship_from_org_id = null
where 1=1
and order_number = 84083
and creation_Date > sysdate - 4

SELECT COUNT (WDD.CONTAINER_NAME)
           FROM APPS.WSH_DELIVERY_DETAILS WDD,
                APPS.WSH_DELIVERY_ASSIGNMENTS WDA
          WHERE     WDA.DELIVERY_ID = 1097862478
                AND WDD.DELIVERY_DETAIL_ID = WDA.DELIVERY_DETAIL_ID;
                
         SELECT NVL (wc.scac_code, wcs.attribute3)
           FROM apps.wsh_carrier_services wcs,
                apps.hz_parties hp,
                apps.wsh_carriers wc
          WHERE     1 = 1
                AND wcs.enabled_flag = 'Y'
                AND hp.party_id = wcs.carrier_id
                AND hp.status = 'A'
                AND wc.carrier_id = hp.party_id
                AND wc.freight_code = 'AGS'
                AND ROWNUM = 1;
